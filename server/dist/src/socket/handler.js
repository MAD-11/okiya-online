...
function setupSocket(io, loadedGames) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('create_room', (data, callback) => {
            logger_1.logger.info(`create_room from ${socket.id} with maxWins: ${data.maxWins}, playerId: ${data.playerId}, nick: ${data.nick}`);
        ...
        socket.on('join_room', (data, callback) => {
            logger_1.logger.info(`join_room from ${socket.id} for room ${data.roomId}, playerId: ${data.playerId}, nick: ${data.nick}`);
                    ...
                    const timer = setTimeout(() => {
                        ...
                        if (currentGame && currentGame.status === 'playing') {
                            if (currentGame.players.red === socket.id || currentGame.players.black === socket.id) {
                                ...
                                (0, redis_1.saveGame)(roomId, currentGame);
